...
if __name__ == "__main__":
    ...
    time_embedding_options = ["sinusoidal", "learnable", "linear", "zero"]

    for time_emb in time_embedding_options:
        experiment_name = f"{args.experiment_name}_time{time_emb}"
        ...
        for epoch in range(args.num_epochs):
            model.train()
            progress_bar = tqdm(total=len(dataloader), desc=f"Epoch {epoch} [time_emb={time_emb}]")
        ...
        with open(f"{outdir}/config.txt", "w") as f:
            f.write(f"time_embedding: {time_emb}\n")
        ...
        plt.plot(losses)
        plt.title(f"Training Loss - time_emb={time_emb}")
        plt.xlabel("Step")
        plt.ylabel("MSE Loss")
